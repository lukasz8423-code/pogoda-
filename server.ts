...
app.get(["/api/weather", "/api/pogoda"], async (req, res) => {
  ...
  try {
    ...
    // Determine soil moisture from Open-Meteo land-surface model (raw 0-1cm moisture)
    ...
    if (weatherData.hourly && Array.isArray(weatherData.hourly.soil_moisture_0_to_1cm) && weatherData.hourly.soil_moisture_0_to_1cm.length > 0) {
      ...
      if (weatherData.current?.time) {
        ...
        idx = times.findIndex((t: string) => t.startsWith(timePrefix));
      }
      if (idx === -1) {
        const nowIsoHour = new Date().toISOString().slice(0, 13);
        idx = times.findIndex((t: string) => t.startsWith(nowIsoHour));
      }

      const sm0 = (idx >= 0 && typeof sm0Arr[idx] === 'number') ? sm0Arr[idx] : undefined;
      if (typeof sm0 === 'number' && !isNaN(sm0)) {
        wilgotnoscSatelitarna = Math.round((sm0 <= 1.0 ? sm0 * 100 : sm0) * 10) / 10;
      }
  ...
  if (mode === "podlej") {
    ...
    if (typeof wilgotnoscSatelitarna !== "number" || isNaN(wilgotnoscSatelitarna)) {
      return {
        ...
        clothes: "Strój roboczy do ogrodu",
...
// API Route: Get current app URL dynamically for the QR code share functionality
