...
export default function MainWeather({ data, userLat, userLng, onRefresh, onBackToSearch, isRefreshing, onLocationSelected, geoDiagnostic }: MainWeatherProps) {
  ...
  const isRainWeatherCode = [51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82, 95, 96, 99].includes(wCode);
  ...
  // Algorytm szansy opadów dla dnia dzisiejszego:
  // W prognozie interesuje nas szansa opadów na POZOSTAŁĄ część dzisiejszego dnia (od bieżącej godziny do końca doby),
  // a nie archiwalna wartość z nocy (np. o 01:00 rano), która już minęła!
  const isActivelyRaining = (current?.precipitation || 0) > 0.05 || isRainWeatherCode;

  let todayRemainingHourlyPopMax: number | null = null;
  let todayWholeDayHourlyPopMax: number | null = null;

  if (Array.isArray(hourly?.time) && Array.isArray(hourly?.precipitation_probability)) {
    const remainingPops: number[] = [];
    const allTodayPops: number[] = [];
    const startIdx = currentIdx >= 0 ? currentIdx : 0;

    hourly.time.forEach((t: string, i: number) => {
      if (typeof t === 'string' && t.startsWith(todayDatePrefix)) {
        const val = hourly.precipitation_probability[i];
        if (typeof val === 'number' && !isNaN(val)) {
          allTodayPops.push(val);
          if (i >= startIdx) {
    ...
    if (remainingPops.length > 0) {
      todayRemainingHourlyPopMax = Math.max(...remainingPops);
    }
    if (allTodayPops.length > 0) {
      todayWholeDayHourlyPopMax = Math.max(...allTodayPops);
    }
  }

  // Obliczenie docelowej szansy opadu na dziś (prognoza w przód):
  let resolvedTodayPopMax: number;
  let popMaxSelectionSource = 'Prognoza na dziś (od bieżącej godziny)';

  if (isActivelyRaining) {
    resolvedTodayPopMax = Math.max(90, currentPop ?? 95);
    popMaxSelectionSource = 'Aktywne opady / deszcz na stacji';
  } else if (todayRemainingHourlyPopMax !== null) {
    resolvedTodayPopMax = todayRemainingHourlyPopMax;
    popMaxSelectionSource = `MAX(hourly.precipitation_probability[od godz. ${currentIdx}])`;
  } else if (currentPop !== null) {
    resolvedTodayPopMax = currentPop;
    popMaxSelectionSource = 'Bieżąca godzina (hourly.precipitation_probability)';
  } else if (todayWholeDayHourlyPopMax !== null) {
    resolvedTodayPopMax = todayWholeDayHourlyPopMax;
    popMaxSelectionSource = 'Maksimum dobowe z godzin';
  } else if (primaryDailyPopMax !== null) {
    resolvedTodayPopMax = primaryDailyPopMax;
    popMaxSelectionSource = `daily.precipitation_probability_max[${todayDailyIndex}]`;
  } else {
    resolvedTodayPopMax = 0;
  }
  ...
  const opticalCloudLabel = getOpticalCloudDescription(opticalCloudCover, { low: lowCloud, mid: midCloud, high: highCloud, total: currentCloudCover });
                ...
                >
                  ...
                  <div className={`flex flex-col items-center mt-2 px-2.5 py-0.5 rounded-full border ${
                    ...
                    <div className="flex items-center text-[10px] font-bold">
                      <Droplets className="w-2.5 h-2.5 mr-0.5 text-cyan-300" />
                      {hour.pop}%
                    </div>
                    {hour.precip > 0 && (
          ...
          <section className="space-y-4 max-w-4xl mx-auto mb-10">
            ...
            <div className="space-y-3">
              {daily.time.slice(todayDailyIndex, todayDailyIndex + 3).map((day, offsetIdx) => {
                ...
                const dayWCode = offsetIdx === 0 && current?.weather_code !== undefined
                  ? current.weather_code
                  : (daily.weather_code?.[actualDailyIdx] ?? 0);
                const dMeta = getWeatherMeta(
                  dayWCode, 
                  offsetIdx === 0 ? isDay : true, 
                  offsetIdx === 0 ? currentCloudCover : undefined, 
                  offsetIdx === 0 ? (currentPrecipitation || 0) : undefined
                );
                const isExpanded = expandedDayIndex === 'all' || expandedDayIndex === offsetIdx;
                const dayPop = offsetIdx === 0 
                  ? resolvedTodayPopMax 
                  : (typeof daily.precipitation_probability_max?.[actualDailyIdx] === 'number' ? daily.precipitation_probability_max[actualDailyIdx] : 0);
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="p-1 rounded-xl bg-white/[0.04]">
                          <AiWeatherIcon 
                            code={dayWCode}
                            isDay={offsetIdx === 0 ? isDay : true}
                            cloudCover={offsetIdx === 0 ? currentCloudCover : 50}
                            className="w-8 h-8"
                          />
                        </div>
                ...
                >
                  {current ? (
                    <>
                      ...
                      <SatelliteStatusCard 
                        locationName={city}
                        soilMoistureSat={current?.soil_moisture_satellite ?? null}
