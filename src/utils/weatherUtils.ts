...
/**
 * Format measurement time from IMGW (e.g. "2026-08-28 12:00:00" UTC -> "14:00" in Warsaw, "12" -> "12:00")
 */
export function formatMeasurementHour(measurementTime: string | null | undefined): string {
  ...
  // If full date/timestamp format (e.g. "2026-09-14 14:10:00" or ISO), IMGW transmits UTC.
  // We format it into Poland's local timezone (Europe/Warsaw) so the user sees real local time.
...
export function getExpectedNextUpdateTime(measurementTime: string | null | undefined): { nextUpdateStr: string; isPastExpectedTime: boolean } {
  ...
  // Sprawdzamy czy obecny czas w strefie Europe/Warsaw minął już przewidywane :25
  const now = new Date();
  ...
  const nowH = parseInt(warsawParts.find(p => p.type === 'hour')?.value || '0', 10);
  const nowM = parseInt(warsawParts.find(p => p.type === 'minute')?.value || '0', 10);
  const currentMinutesFromMidnight = nowH * 60 + nowM;
  const expectedMinutesFromMidnight = nextH * 60 + 25;

  let isPastExpectedTime = false;
  if (nextH === 0 && nowH === 23) {
    isPastExpectedTime = false;
  } else if (nowH === nextH) {
    isPastExpectedTime = nowM > 25;
  } else if (currentMinutesFromMidnight > expectedMinutesFromMidnight && currentMinutesFromMidnight - expectedMinutesFromMidnight < 12 * 60) {
...
export function getCalibratedTemperatureDetails(
  ...
  if (typeof imgwStationOrTemp === 'object' && imgwStationOrTemp !== null) {
    ...
    measurementTime = (imgwStationOrTemp as any).tempMeasurementTime
      ...
      || (imgwStationOrTemp as any).rawMeasurementTime
      || imgwStationOrTemp.measurementTime
      || imgwStationOrTemp.lastSync
      || null;
  ...
  const selectedSource = calibrationMode === 'FRESH_IMGW'
    ? `IMGW Telemetria bezpośrednia (${stationName || 'Stacja'})`
    : (isCalibrated
  ...
  console.log(`📡 [AURA IMGW FRESHNESS AUDIT]`, {
    ...
    measurementTime: measurementTime || 'Brak',
    measurementHourWarsaw: hourStr || 'Brak',
    ageInMinutes: minutesOld,
    freshnessState: calibrationMode,
    isDelayed,
    selectedCurrentTemperatureSource: selectedSource,
    selectedCurrentTemperatureValue: calculatedTemp !== null ? `${Number(calculatedTemp.toFixed(1))}°C` : 'Brak'
  });
 ...
 */
export function getOpticalCloudDescription(
  opticalPercent: number,
  layers?: { low?: number | null; mid?: number | null; high?: number | null; total?: number | null }
): string {
  if (layers) {
    const low = layers.low ?? 0;
    const mid = layers.mid ?? 0;
    const high = layers.high ?? 0;
    const total = layers.total ?? 0;

    // Jeżeli niebo jest pokryte głównie przezroczystymi chmurami wysokimi (Cirrus),
    // a chmury niskie i średnie są znikome, słońce wyraźnie prześwituje mimo wysokiego zachmurzenia modelowego.
    if (high >= 85 && low <= 10 && mid <= 15 && total >= 80) {
      return "Wysokie chmury (słońce prześwituje)";
    }
  }
